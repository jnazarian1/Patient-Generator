...
#The script should not take more than 40 seconds, but I set it to 70 seconds in case the server is slow
begin
...
if patientDrinkingStatus == 2
  unless conditionIndex == 14 || 40 || 45 || 9 #These condition indexes are not to be overwritten- Emphysema/LungCancer/UpperRespiratoryTractDisease/Bronchitis
    ...
    if liverDiseaseChance == 3
      ...
      conditionIndex = 76 #This is the condition index for Alcoholic Cirrhosis of Liver
    end
  end
end
  ...
  if postOperativeVar == false
    if allProcedures.count > 0
      if rand(20) == 7 #This just creates a 5% chance that a procedure will cause a "Post-Operative Infection"
        ...
        conditionIndex = 79 #This is the condition index for "Post-Operative Infection"
...
if deathChance < 100*conditionRepository[conditionIndex][:mortalityChance]
...
allExtraEncounters = []
#The fllowing section of code creates a checkup encounter if the condition requires such
...
#Below is the end of the large 'until' loop that creates the conditions and their corresponding medications amedication statements
end
...
allMedications.each do |medication|
  if medication.kind.nil?
    #The following line adds the medication to an array called "deletedMeds" before actually deleting it-- this was just for debugging purposes, but I will leave it in case it comes in handy later
    deletedMedications.push(medication)
    #For some reason, the following line does not delete some medications that should be deleted. I got stuck on this for a while, so I fixed the bug later on in the script by not printing the blank medications to JSON and therfore noe uploading them to the server
    #Let it be clear that some place-holding medications DO slip through this deleter (although this deleter does catch some-- it's weird)
    allMedications.delete(medication)
    #The following line adds the index of the Medication that is being deleted so the Medication Statements of the same indexes will be deleted
    deletedMedIndices.push(medDeleterCounter)
  end
  #The following series of code deletes duplicate medications because medications do not have dates and they are not related to medicationsStatements through references
  for i in 0..medDeleterCounter
    if medication == allMedications[i-1]
      allMedications.delete(medication)
    end
  end
  #The following line is placed outside the 'if' statement so the counter will increase even if the medication is not deleted
  medDeleterCounter += 1
end

#The following iteration deletes all the blank, placeholding medication statements
deletedMedIndices.each do |index|
  #The following line adds the medication statement to an array called "deletedMedStatements" before actually deleting it-- this was just for debugging purposes, but I will leave it in case it comes in handy later
  deletedMedicationStatements.push(allMedicationStatements[index])
  #The following line actually deletes the placeholding medication statement
  allMedicationStatements.delete(allMedicationStatements[index])
end
...
#The following block of code selects all conditions/medicationStatements/encounters that occur BEFORE the patient has died
...
deletedMedStatements = allMedicationStatements.select {|medicationStatement| medicationStatement.whenGiven.start >= mockPatient.deceasedDateTime}
...
until extraEncounterCounter == numberOfExtraEncounters
  ...
  allExtraEncounters.push(mockExtraEncounter)
end
...
patientHeight.issued = observationAssignmentDate
patientWeight.issued = observationAssignmentDate
patientBMI.issued = observationAssignmentDate
patientAge.issued = observationAssignmentDate
...
if allExtraEncounters.count == 0
  patientSmokingStatus.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientDrinkingStatus.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientHDL.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientLDL.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientTriglyceride.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientFallingHistory.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  patientFallingRiskTest.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
else
  ...
  patientFallingHistory.issued = allExtraEncounters[rand(allExtraEncounters.count-1)].period.start + rand(10..14).hours + rand(0..59).minutes
  patientFallingRiskTest.issued = allExtraEncounters[rand(allExtraEncounters.count-1)].period.start + rand(10..14).hours + rand(0..59).minutes
...
if hasDiabetes == true
  if hasHypertension == true
    patientGlucose.issued = $allConditions[1].dateAsserted - rand(1..2).hours + rand(30..59).minutes
  else
    patientGlucose.issued = $allConditions[0].dateAsserted - rand(1..2).hours + rand(30..59).minutes
  end
else
  if allExtraEncounters.count == 0
    patientGlucose.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  else
...
if hasHypertension == true
  ...
  if allExtraEncounters.count == 0
    patientSystolicBloodPressure.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
    patientDiastolicBloodPressure.issued = allEncounters[0].period.start + rand(10..14).hours + rand(0..59).minutes
  else
...
#This code deletes all other files from previously generated patients to avoid errors
...
jsonPatient = File.open('samplePatient.json', 'w') { |patient|
  ...
  patient << '  "use": "home"' << "\n"
  patient << '  }],' << "\n"
#The following line is the format needed by the SMART on FHIR server
  patient << '"gender": "' << mockPatient.gender.coding[0].display << '",' << "\n"
#The following code is used in place of the previous line for the IE server
#  patient << '"gender": {' << "\n"
#  patient << '  "coding": [{' << "\n"
...
#  patient << '  "text": "' << mockPatient.gender.text << '"},' << "\n"
  ...
  patientPostVar = RestClient.post "http://localhost:8080/Patient", openFile,  :content_type => :json, :accept => :json
  $patientIDVar = patientPostVar[40..63]
  patientIDString = "http://localhost:8080/Patient/" << patientPostVar[40..63]
  patientResponse = RestClient.get patientIDString.to_s
  $postedFiles << patientIDString
...
unless allMedications.count == 0
  ...
  medicationPossibleFileNames.each do |fileNameBase|
    ...
    localURL = "http://localhost:8080/" << "#{fileNameBase[:type]}"
    ...
    if File.file?(fileName)
      ...
      medicationIteratorPostVar = RestClient.post "http://localhost:8080/Medication", openFile, :content_type => :json, :accept => :json
      medicationIteratorPostID = medicationIteratorPostVar[42..67]
      medicationIDString = "http://localhost:8080/Medication/" << medicationIteratorPostVar[43..66]
      medicationIDsForReferences << medicationIteratorPostVar[43..66]
      medicationResponse = RestClient.get medicationIDString.to_s
      $postedFiles << medicationIDString
  ...
  until medicationStatementPrinterCounter == allMedicationStatements.count-1
    ...
    if allMedicationStatements[medicationStatementPrinterCounter].medication.present?
      ...
      jsonMedicationStatement = File.open(medicationStatementFileNameVar, 'w') { |medicationStatement|
        ...
        medicationStatement << '  "reference": "Medication/' << "#{medicationIDsForReferences[medicationStatementPrinterCounter]}" << '",' << "\n"
        medicationStatement << '  "display": "' << allMedications[medicationStatementPrinterCounter].name << '"},' << "\n"
        medicationStatement << '"dateAsserted": "' << allMedicationStatements[medicationStatementPrinterCounter].whenGiven.start.to_s[0..9] << '",' << "\n"
        if $allConditions[medicationStatementPrinterCounter].abatementBoolean == true
          jsonMedicationStatement = File.open(medicationStatementFileNameVar, 'w') { |medicationStatement|
            medicationStatement << '"effectivePeriod": {' << "\n"
            medicationStatement << '  "start": "' << allMedicationStatements[medicationStatementPrinterCounter].whenGiven.start.to_s[0..9] << '",' << "\n"
            medicationStatement << '  "end": "' << allMedicationStatements[medicationStatementPrinterCounter].whenGiven.end.to_s[0..9] << '"},' << "\n"
          }
        end
...
encounterFileNameVar = "sampleEncounter" << encounterFilePrinterCounter.to_s << ".json"
  ...
  jsonEncounter = File.open(encounterFileNameVar, 'w') { |encounter|
    ...
    puts "THERE WAS AN ERROR printing the coding portion of Encounter#{encounterFilePrinterCounter}\n"
    puts "Is allEncounters[encounterPrinterCounter] blank: #{allEncounters[encounterPrinterCounter].empty?}\n"
    if allEncounters[encounterPrinterCounter].empty?
      puts "allEncounters[encounterPrinterCounter] is not defined\n"
    else
      puts "allEncounters[encounterPrinterCounter] contains the following:\n#{allEncounters[encounterPrinterCounter]}\n"
      if allEncounters[encounterPrinterCounter].reason.empty?
        puts "allEncounters[encounterPrinterCounter].reason is not defined\n"
      else
        puts "allEncounters[encounterPrinterCounter].reason contains the following:\n#{allEncounters[encounterPrinterCounter].reason}\n"
        if allEncounters[encounterPrinterCounter].reason.text.empty?
          puts "allEncounters[encounterPrinterCounter].reason.text is not defined\n"
        else
          puts "allEncounters[encounterPrinterCounter].reason.text contains the following:\n#{allEncounters[encounterPrinterCounter].reason.text}\n"
        end
      end
    end
...
#This code loops throught the above array and posts each json file to the local SMART on FHIR server
 ...
 encounterPossibleFileNames.each do |fileNameBase|
  ...
  if File.file?(fileName)
    openFile = File.read(fileName)
    encounterIteratorPostVar = RestClient.post "http://localhost:8080/Encounter", openFile, :content_type => :json, :accept => :json
    encounterIteratorPostID =  encounterIteratorPostVar[41..66]
    encounterIDString = "http://localhost:8080/Encounter/" <<  encounterIteratorPostVar[42..65]
    encounterIDsForReferences <<  encounterIteratorPostVar[42..65]
    encounterResponse = RestClient.get  encounterIDString.to_s
    $postedFiles << encounterIDString
    #This lines prints the name, HTTP response code, and ID value of each generated resource to the terminal
...
conditionFileNameVar = "sampleCondition" << conditionFilePrinterCounter.to_s << ".json"
  jsonCondition = File.open(conditionFileNameVar, 'w') { |condition|
    ...
    condition << '"dateAsserted": "' << $allConditions[conditionPrinterCounter].dateAsserted.to_s[0..9] << '",' << "\n"
    condition << '"code": {' << "\n"
    condition << '  "coding": [{' << "\n"
...
jsonSmokingStatus = File.open('sampleSmokingStatus.json', 'w') { |smokingStatus|
  ...
  smokingStatus << '"issued": "' << patientSmokingStatus.issued << '",' << "\n"
...
jsonDrinkingStatus = File.open('sampleDrinkingStatus.json', 'w') { |drinkingStatus|
  ...
  drinkingStatus << '"issued": "' << patientDrinkingStatus.issued << '",' << "\n"
...
jsonSystolicBloodPressure = File.open('sampleSystolicBloodPressure.json', 'w') { |systolicBloodPressure|
  ...
  systolicBloodPressure << '"issued": "' << patientSystolicBloodPressure.issued << '",' << "\n"
...
jsonDiastolicBloodPressure = File.open('sampleDiastolicBloodPressure.json', 'w') { |diastolicBloodPressure|
  ...
  diastolicBloodPressure << '"issued": "' << patientDiastolicBloodPressure.issued << '",' << "\n"
...
jsonLDLCholesterol = File.open('sampleLDLCholesterol.json', 'w') { |lDLCholesterol|
  ...
  lDLCholesterol << '"issued": "' << patientLDL.issued << '",' << "\n"
...
jsonHDLCholesterol = File.open('sampleHDLCholesterol.json', 'w') { |hDLCholesterol|
  ...
  hDLCholesterol << '"issued": "' << patientHDL.issued << '",' << "\n"
...
jsonTriglyceride = File.open('sampleTriglyceride.json', 'w') { |triglyceride|
  ...
  triglyceride << '"issued": "' << patientTriglyceride.issued << '",' << "\n"
...
jsonHeight = File.open('sampleHeight.json', 'w') { |height|
  ...
  height << '"issued": "' << patientHeight.issued << '",' << "\n"
  height << '"subject": {' << "\n"
...
jsonWeight = File.open('sampleWeight.json', 'w') { |weight|
  ...
  weight << '"issued": "' << patientWeight.issued << '",' << "\n"
  weight << '"subject": {' << "\n"
...
jsonGlucose = File.open('sampleGlucose.json', 'w') { |glucose|
  ...
  glucose << '"issued": "' << patientGlucose.issued << '",' << "\n"
  glucose << '"subject": {' << "\n"
...
jsonFallingHistory = File.open('sampleFallingHistory.json', 'w') { |fallingHistory|
  ...
  fallingHistory << '"issued": "' << patientFallingHistory.issued << '",' << "\n"
...
jsonFallingRiskTest = File.open('sampleFallingRiskTest.json', 'w') { |fallingRiskTest|
  ...
  fallingRiskTest << '"issued": "' << patientFallingRiskTest.issued << '",' << "\n"
...
if mockPatient.deceasedBoolean == true
  jsonCauseOfDeath = File.open('sampleCauseOfDeath.json', 'w') { |causeOfDeath|
    ...
    causeOfDeath << '"issued": "' << mockPatient.deceasedDateTime.to_datetime << '",' << "\n"
...
#Patient is not include in this array because the patient resource is posted before the rest, so its ID can be used for references
possibleFileNames = [
...
possibleFileNames.each do |fileNameBase|
  ...
  if File.file?(fileName)
    openFile = File.read(fileName)
    iteratorPostVar = RestClient.post localURL, openFile, :content_type => :json, :accept => :json
    response = RestClient.get localURL
    #This code just determines what characters from the HTTP post request are printed to the terminal to show the ID's of each generated resource
    case fileNameBase[:type]
    when "AllergyIntolerance"
      iteratorPostID = iteratorPostVar[50..75]
      allergyIntoleranceIDString = "http://localhost:8080/AllergyIntolerance/" <<  iteratorPostID[2..24]
      $postedFiles << allergyIntoleranceIDString
    when "Observation"
      iteratorPostID = iteratorPostVar[43..68]
      observationIDString = "http://localhost:8080/Observation/" <<  iteratorPostID[2..24]
      $postedFiles << observationIDString
    when "Condition"
      iteratorPostID = iteratorPostVar[41..66]
      conditionIDString = "http://localhost:8080/Condition/" <<  iteratorPostID[2..24]
      $postedFiles << conditionIDString
    when "MedicationStatement"
      iteratorPostID = iteratorPostVar[51..76]
      medicationStatementIDString = "http://localhost:8080/MedicationStatement/" <<  iteratorPostID[2..24]
      $postedFiles << medicationStatementIDString
    when "Procedure"
      iteratorPostID = iteratorPostVar[41..66]
      procedureIDString = "http://localhost:8080/Procedure/" <<  iteratorPostID[2..24]
      $postedFiles << procedureIDString
    end
    #This lines prints the name, HTTP response code, and ID value of each generated resource to the terminal
    puts "The #{fileNameBase[:type]} ID for #{fileNameBase[:name]} (HTTP response code: #{response.code}) is #{iteratorPostID}"
  end
end
puts "----------------------------------------------------------------------------------------------------\n"
...
#This code just exits the script and deletes all resources that were posted before to the server before it got stuck (so there are not like half-patients on the server)
  ...
  if defined?($postedFiles)
    if $postedFiles.present?
      ...
      puts "There was a timeout error, but no files were posted to the local SMART ON FHIR server\n"
    end
  end
end
